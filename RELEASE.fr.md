...
| | |
| :-- | :-- |
| Actuelle | **0.1.4** (`CFBundleShortVersionString` dans `Sources/SiftApp/Info.plist`, build `5`) |
...
Le script compile le binaire de release, l’enveloppe dans `Sift.app`, copie `Info.plist`, et écrit `.build/distribution/Sift.dmg`. L’image disque s’appelle `Sift` plus la version de `Info.plist` (`Sift 0.1.4`) et contient l’app plus un raccourci vers Applications. L’identifiant du bundle est `ai.cyclones.sift` par défaut.
...
Depuis la copie `sift`, une fois le commit 0.1.4 sur `main` :

```bash
git tag -a v0.1.4 -m "Sift v0.1.4"
git push origin main
git push origin v0.1.4
gh release create v0.1.4 .build/distribution/Sift.dmg \
  --repo Cyberesia/sift \
  --title "Sift v0.1.4" \
  --notes-file CHANGELOG.md
```

Gardez le `Sift.dmg` de la version précédente sur sa release (0.1.3 tant que la 0.1.4 est l’actuelle). Les releases plus anciennes gardent leurs notes et n’ont pas de DMG. Ne déplacez pas un tag existant. Collez la section `0.1.4` de [CHANGELOG.fr.md](./CHANGELOG.fr.md) si vous préférez les notes en français. L’anglais est dans [CHANGELOG.md](./CHANGELOG.md).
