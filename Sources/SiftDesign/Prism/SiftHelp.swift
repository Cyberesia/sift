...
enum SiftHelpArticles {
    static let list: [SiftHelpArticle] = [
        ...
        article("discover-scan-dialog", ["sources"], "Dialog: Scan this Mac", "Dialogue : Scanner ce Mac",
                "Scan this Mac does not start immediately. It opens Find files on this Mac. The first line names the kinds you turned on. The next line says this only builds a catalog and files stay in their folders. If Full Disk Access is on, the window says it will check home folders and mounted disks, and offers Choose folders and Scan this Mac. If it is off, it says Choose folders works now, and Enable Full Disk Access opens the system setting. Cancel closes the window and scans nothing. A note lists what is always skipped: macOS system trees, apps, caches, .git, node_modules, DerivedData, .build, Pods, and package bundles.",
        ...
        article("discover-look-again", ["sources"], "Look again", "Look again",
                "Look again appears only after at least one folder is saved. The button’s help names those folders before you click. Clicking it opens Look again? The message names the folders and says Sift walks them and updates the catalog, and files stay where they are. Confirm is Look again. Cancel walks nothing. A later change inside a saved folder, when Watch source folders is on, checks only the files that changed. It does not walk the whole folder.",
        ...
        article("discover-locations", ["sources"], "Catalog locations", "Emplacements du catalogue",
                "Catalog locations lists each saved folder. Includes subfolders means the scan walks inside. This folder only means it does not. Change… picks a replacement and rebuilds that part of the catalog. Remove opens Remove “name”? The message says the folder is forgotten as a source, files already there stay on the Mac, and they disappear from the catalog. Confirm is Remove from catalog. Cancel leaves the source in place. The same row is in Settings → Source folders.",
        ...
        article("library-views", ["library"], "Grid, list, and Garden", "Grille, liste et Garden",
                "The same catalog has three views. Grid is the mosaic. List is one row per file, with the filename and up to three labels. Garden is the spatial view: scroll or drag to turn, click a photo to inspect. Switching views does not restart a scan. The date on a tile is the day the file entered the catalog, not the day the camera took it.",
                "Le même catalogue a trois vues. Grille est la mosaïque. Liste est une ligne par fichier, avec le nom et jusqu’à trois labels. Garden est la vue spatiale : défiler ou glisser pour tourner, cliquer une photo pour l’inspecter. Changer de vue ne relance pas un scan. La date sur une tuile est le jour d’entrée dans le catalogue, pas le jour de la prise de vue.",
                ["grid", "list", "garden"]),
        article("library-cards", ["library"], "Hover card and players", "Carte au survol et lecteurs",
                "Hovering a photo or video opens its card without opening the viewer. A click opens the viewer. Audio tiles show a waveform and the filename, and the card has a player that does not start by itself. Video plays in the card. Documents, including docx, pptx, xlsx, and pdf, open in Quick Look in the card and in the viewer. The card also shows the filename, the source folder, and the date the file was added.",
                "Survoler une photo ou une vidéo ouvre sa carte sans ouvrir le grand lecteur. Un clic ouvre le lecteur. Les tuiles audio montrent une forme d’onde et le nom du fichier, et la carte a un lecteur qui ne démarre pas seul. La vidéo se lit dans la carte. Les documents, y compris docx, pptx, xlsx et pdf, s’ouvrent dans Quick Look dans la carte et dans le lecteur. La carte montre aussi le nom, le dossier source, et la date d’ajout.",
                ["hover", "quick look", "player"]),
        article("library-sidebar", ["library"], "Sidebar", "Barre latérale",
                "The sidebar lists media kinds and a count for each. Choosing one filters the library. Named people lists only people you accepted in Review AI. A row shows the name and how many photos are in the group. Naming and dismissing happen in Review AI, not here.",
                "La barre latérale liste les types de médias et un compte pour chacun. En choisir un filtre la bibliothèque. Personnes nommées ne liste que les personnes acceptées dans Revue. Une ligne montre le nom et combien de photos sont dans le groupe. Nommer et écarter se font dans Revue, pas ici.",
        ...
        article("settings-privacy", ["settings"], "Privacy", "Confidentialité",
                "Privacy has two rows: All processing is on-device, and No network required. The footer says your library and Vision models never leave this Mac. Photos, video, and audio are not uploaded. The one exception is under TypeSafe System One → Why?: if a Jev key is saved, indexing a document can send the opening lines, headings, sheet names, and column headers once. Return in search can send the phrase and a short list of choices. The file itself stays on this Mac. With no key, that call is not made.",
                "Confidentialité a deux lignes : Tout le traitement est sur l’appareil, et Aucun réseau requis. Le pied de page dit que votre bibliothèque et les modèles Vision ne quittent jamais ce Mac. Photos, vidéo et audio ne sont pas envoyés. La seule exception est sous TypeSafe System One → Pourquoi ? : si une clé Jev est enregistrée, l’indexation d’un document peut envoyer une fois les premières lignes, les titres, les noms de feuilles et les en-têtes de colonnes. Entrée dans la recherche peut envoyer la phrase et une courte liste de choix. Le fichier lui-même reste sur ce Mac. Sans clé, cet appel n’est pas fait.",
