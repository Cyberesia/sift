...
| | |
| :-- | :-- |
| Current | **0.1.4** (`CFBundleShortVersionString` in `Sources/SiftApp/Info.plist`, build `5`) |
...
The script builds the release binary, wraps it as `Sift.app`, copies `Info.plist`, and writes `.build/distribution/Sift.dmg`. The disk image is named `Sift` plus the version in `Info.plist` (`Sift 0.1.4`) and contains the app plus a shortcut to Applications. The bundle identifier defaults to `ai.cyclones.sift`.
...
From the `sift` checkout, after the 0.1.4 commit is on `main`:
...
Keep the previous version's `Sift.dmg` on its release (0.1.3 while 0.1.4 is current). Older releases keep their notes and do not keep a DMG. Do not move an existing tag. Paste the `0.1.4` section from [CHANGELOG.md](./CHANGELOG.md) if you would rather not attach the whole file. The French notes are in [CHANGELOG.fr.md](./CHANGELOG.fr.md).
