...
| | |
| :-- | :-- |
| Current | **0.1.1** (`CFBundleShortVersionString` in `Sources/SiftApp/Info.plist`, build `2`) |
| First GitHub Release | 0.1.1, asset `Sift.dmg` |
...
The script builds the release binary, wraps it as `Sift.app`, copies `Info.plist`, and writes `.build/distribution/Sift.dmg`. The disk image is named `Sift 0.1.1` and contains the app plus a shortcut to Applications. The bundle identifier defaults to `ai.cyclones.sift`.
...
0.1.0 was never published there as a Release. There is no previous DMG to replace. 0.1.1 is the first one. The in-app updater reads `https://api.github.com/repos/Cyberesia/sift/releases/latest` and downloads the `.dmg` asset.

From the `sift` checkout, after the 0.1.1 commit is on `main`:
...
Paste the `0.1.1` section from [CHANGELOG.md](./CHANGELOG.md) if you would rather not attach the whole file. The French notes are in [CHANGELOG.fr.md](./CHANGELOG.fr.md).
